...
- Both controls sit in the left gutter, stacked as one column (`.reader-block-actions`, positioned once at `calc(-1 * var(--reader-gutter))`): edit above, anchor below. The buttons themselves are static in flow, so the pair cannot drift apart.
- The anchor control used to hang at the end of the measure, which is inside the block box: a table or a wide code fence is allowed to run past the prose measure, and the control landed on top of that content. Out in the gutter neither control can overlap the document. Verified in the build at 1280px: the pair ends at `x: 234`, the table starts at `x: 240`, and the two buttons share `x: 209` at `y: 200` / `y: 230`.
...
- **Granular targeting inside a list or a table.** A double-click resolves the pointer to the Sub-block it landed on (`subBlockTargetFromNode`, `src/webview/app/markdown/sub-blocks.ts`), so a double-click on one list item opens that item and not the forty-item list around it. The innermost item wins, so a nested item is editable on its own; aiming at the parent's own line takes the whole subtree, since an item's range covers its children. A list item is addressed by path (`[2, 0]` is the third item's first child), a table row by number with the header at 0; the alignment delimiter is not a row and can never be opened.
  - Targets are positional and live only as long as the editor (`SubBlockTarget`, `packages/domain/services/sub-blocks.ts`). Nothing persisted gains a shape: an Anchor is still block-level, so Notes, Suggestions and the MCP surface are untouched by an item becoming editable.
  - The splice is the Sub-block's own source range (`applySubBlockEdit`), so the rest of the list, the table's other rows and the blank lines between them survive byte for byte, markers and alignment padding included.
  - The rest of the block stays on screen around the editor, cut at the Sub-block's own boundaries out of the block's source (`splitAroundSubBlock`), so an ordered list's tail carries on counting from the author's own markers and a sibling of an edited nested item keeps its indentation. **Known compromise**: the tail of a split table repeats the header and alignment row, because rows without them do not render as a table. It only shows while the editor is open.
  - The gutter control and the menu's `Edit block` stay whole-block: that is the block's own affordance, and the only route that never needs a pointer. The menu gains `Edit item` / `Edit row` for the part the right-click landed on.
