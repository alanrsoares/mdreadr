...
/**
 * The Model Context Protocol mark, in the shape `Icon` accepts from heroicons:
 * a 24-unit square that takes its size and colour from the props it is spread.
 *
 * Hand-held rather than pulled from an icon set because the app's set
 * (heroicons) has no protocol logos, and a cog for the MCP settings button
 * reads as the app's own preferences instead.
 */
export function McpIcon(props: SVGProps<SVGSVGElement>) {
  return (
    <svg
      ...
      // Decorative by default, as heroicons are: `Icon` names it when the
      // glyph stands alone, and its props win over this one.
      aria-hidden="true"
      {...props}
    >
