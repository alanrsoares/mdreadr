...
- [x] WS-3 — Journal (`seq`-numbered events) + `wait_for_activity` / `get_events` MCP tools. `SessionStore` gained an append-only `journal`/`nextSeq`/`waiters`, with `addNote`/`addSuggestion`/`noteReplied`/`noteStatusChanged`/`suggestionStatusChanged` each emitting the right event type; all `index.ts` and `mcp.ts` call sites that used to call the bare `replaceNote`/`replaceSuggestion` now go through these event-emitting wrappers (including the MCP `add_reply`/`set_note_status` tools, not just the HTTP routes, since agents mutate primarily through MCP). `wait_for_activity(sinceSeq, timeoutMs = 25000)` and `get_events(sinceSeq)` added as new MCP tools. Deviation: no separate `SessionStore`-level unit test file added — coverage lives in `mcp.test.ts` per the plan's test spec (timeout/immediate-resolve/two-concurrent-waiters), using a `currentMaxSeq()` helper to baseline `sinceSeq` per test since the journal is a module-level singleton not reset between tests. `bun run check` green.
- [x] WS-6 — Proxy reconnect (`mcp.json` watch) + resume (`session.jsonl` + catch-up read). `mcp-stdio-proxy.ts` rewritten: `fs.watch`s `~/.config/mdreadr` for `mcp.json` changes, tears down/recreates the HTTP transport on a URL/token change, and re-reads config once on a send failure in case the watcher raced the write. Snoops `wait_for_activity`/`get_events` responses flowing through it to persist new journal entries to `~/.config/mdreadr/session.jsonl` (append-only, deduped by seq); on reconnect it replays the last `initialize`/`notifications/initialized` handshake against the new transport (a new server means a new MCP session — found via manual smoke test that skipping this makes the SDK reject all follow-up calls with "Server not initialized"), then proactively calls `get_events(lastSeq)` against the new server so a Note added during the restart window isn't dropped. Deviation: internal-only replayed/resume calls are tracked in a separate `internalOnlyIds` set and never forwarded to stdio, since the downstream client has no id to match an unsolicited response against — this wasn't explicit in the plan but is necessary given the plan's own resume design. Manually smoke-tested end-to-end (two real server instances on different ports, watched, reconnected, journal persisted) instead of the heavier fake-HTTP-server test harness the plan flagged as possibly "too heavy for the payoff"; added unit tests instead for the pure helpers (`mergeNewJournalEntries`, `readConfig`, `loadPersistedMaxSeq`/`appendJournalEntries` round-trip) in `mcp-stdio-proxy.test.ts`. `bun run check` green.
- [x] WS-5 — Per-session MCP transport state. Confirmed via SDK source (`webStandardStreamableHttp.d.ts`/`.js`, v1.29.0) that `WebStandardStreamableHTTPServerTransport` is inherently single-session (one `sessionId`/`_initialized` field per instance, keyed off the `mcp-session-id` request header) and `Server.connect()` throws if called twice — so the fix is a session-keyed `Map<sessionId, {server, transport}>` in `mcp.ts`, not a native upstream feature. Extracted tool registration into `registerHandlers(server)` so `createMcpServer()` builds an identical fresh `Server` per session; `createSession()` wires `onsessioninitialized`/`onsessionclosed` to add/remove the map entry; `handleMcpRequest(request)` routes by the `mcp-session-id` header, creating a fresh session for header-less (i.e. initialize) requests. `index.ts`'s `/mcp` and `/mcp/message` handlers now just call `handleMcpRequest` — the global `_initialized`/`sessionId` reset hack is gone entirely. `mcpServer` (handlers registered, never connected to a transport) stays exported for `mcp.test.ts`'s direct-handler-invocation tests. Added a concurrent-initialize test (two un-awaited `/mcp` initialize POSTs → distinct `mcp-session-id` headers → both sessions independently answer `tools/list`). `bun run check` green.
- [x] WS-7 — Path-scope `save_session_notes`. Added `isSaveNotesPathAllowed(targetPath, documentPath, home?)` to `documents.ts`: allowed roots are the open Document's directory (if any), `~/Documents`, `~/Desktop`, and `~` itself, boundary-checked (not a naive prefix match, so `/opt/project-evil` doesn't pass for root `/opt/project`). `home` is an injectable param (defaults to `homedir()`) for testability, matching this session's other testable-pure-function pattern. `mcp.ts`'s `save_session_notes` handler now checks it before writing and returns `{error, code: "PathNotAllowed"}` as tool content instead of writing — no exception thrown, per the plan's spec. Tests: `documents.test.ts` covers the pure function directly (doc-dir, nested, rejected-outside, prefix-collision, Documents/Desktop/home, no-document-open cases); `mcp.test.ts` adds an end-to-end `save_session_notes path scoping` describe block (rejects outside path, saves inside the open Document's directory, using a real `mkdtemp` dir). `bun run check` green.
